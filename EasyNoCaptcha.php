    ...
    class ENCv2
    {
            ...
            $result = '
                ...
                '
                    setTimeout(function() {
                        ...
                        ' . $T['MutationObserver'] . '["observe"](' . $T['document2'] . '["querySelectorAll"]("body")[0],{childList:true,characterData:true,subtree:true});
        ...
        function CheckEasyNoCaptha()
        {
            ...
            if ((isset($_REQUEST['HASHCODE'])) && (isset($_REQUEST['HASH']))) {
                if (($_REQUEST['HASHCODE'] != '') && ($_REQUEST['HASH'] != '')) {
                    if (isset($_SESSION['MYHASH'][$_REQUEST['HASHCODE']])) {
                        $session = $_SESSION['MYHASH'][$_REQUEST['HASHCODE']];
                        $curArray = $this->curArray;
                        unset($_SESSION['MYHASH'][$_REQUEST['HASHCODE']]);
                        if ($session['VALUE'] == $_REQUEST['HASH']) {
                            if ($curArray['IP'] == $session['IP']) {
                                /* IP совпадают */
                                $precheck = true;
                            };
                        };
